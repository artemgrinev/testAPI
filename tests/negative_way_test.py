...
from src.pydantic_schemas.errors import ErrorData, Error
...
class TestNegativeWayUsers:
    ...
    def test_case_dependency_body(self, post):
        ...
        response.assert_status_code(400).validate(ErrorData)
        assert response.json_data['error'] == body_not_valid
        assert response.json_data['data']['lastName'] == 'Path `lastName` is required.'
        assert response.json_data['data']['firstName'] == 'Path `firstName` is required.'
        assert response.json_data['data']['email'] == 'Path `email` is required.'
    ...
    def test_create_user_with_exist_email(self, post):
        ...
        response.assert_status_code(400).validate(ErrorData)
        assert response.json_data['error'] == body_not_valid
        assert response.json_data['data']['email'] == 'Email already used'
    ...
    def test_create_user_missing_auth_token(self, post):
        ...
        response.assert_status_code(403).validate(Error)
        assert response.json_data['error'] == app_id_missing
    ...
    def test_create_user_invalid_auth_token(self, post):
        ...
        response.assert_status_code(403).validate(Error)
        assert response.json_data['error'] == app_id_not_exist
    ...
    def test_update_email(self, put):
        ...
        response.assert_status_code(400).validate(Error)
        assert response.json_data['error'] == params_not_valid
...
class TestNegativeWayDelete:
    ...
    def test_delete_not_existing_comment(self, delete):
        ...
        response.assert_status_code(404).validate(Error)
